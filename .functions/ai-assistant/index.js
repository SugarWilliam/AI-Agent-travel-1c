...
async function generatePlan(input, userId) {
  ...
  const plan = {
    ...
    guide: {
      overview: `${destination}是一个充满魅力的旅游目的地。`,
      highlights: ['著名景点', '特色美食', '文化体验'],
      tips: ['注意防晒', '保持环保', '尊重当地文化']
    },
    photoTips: {
      bestSpots: ['景点A', '景点B'],
      tips: ['最佳拍摄时间：早晨和傍晚', '使用广角镜头']
    },
    outfitTips: {
      recommendations: [
...
async function suggestTimeAdjustment(input) {
  return {
    success: true,
    suggestions: [
      { nodeId: 'node1', originalTime: '09:00', suggestedTime: '10:00', reason: '避开人流高峰' },
      { nodeId: 'node2', originalTime: '14:00', suggestedTime: '15:00', reason: '天气更佳' }
    ]
  };
}
...
async function refreshNodeStatus(input) {
  ...
  return {
    ...
    node: {
      id: nodeId,
      status: 'completed',
      weather: { temperature: '25°C', condition: '晴', icon: '☀️' },
      suggestions: ['建议携带防晒用品', '适合户外活动']
    },
    itinerary: [
      {
        day: 1,
        ...
        title: '抵达与探索',
        activities: [
          { time: '09:00', title: '抵达机场', location: '机场', type: 'transport', status: 'completed' },
          { time: '10:00', title: '入住酒店', location: '酒店', type: 'accommodation', status: 'pending' },
          { time: '14:00', title: '游览市中心', location: '市中心', type: 'sightseeing', status: 'pending' },
          { time: '18:00', title: '晚餐', location: '餐厅', type: 'dining', status: 'pending' }
        ]
      }
    ]
  };
